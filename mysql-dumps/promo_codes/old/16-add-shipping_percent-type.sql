...
-- =============================================================================
-- OLD-PATH copy of new/16-add-shipping_percent-type.sql (identical SQL).
-- Extends the `type` ENUM on `promo_codes` (imartap catalog) and on
-- `cart_promo_codes` (regional after 14; imartap backup copy also covered while
-- it still exists). `order_promo_codes` has no `type` column → not touched.
--
-- Idempotent + table-existence guarded against DATABASE(): re-running, or running
-- against a DB lacking a given table, is a no-op. Run against imartap AND each
-- regional DB. Order: after 14 (so the regional cart_promo_codes exists).
-- =============================================================================
