...
--
-- Idempotent + table-existence safe (mirrors 07-add-item-porachki-columns.sql):
-- each ALTER runs only when the table exists in DATABASE() AND the column is
-- missing; the backfill runs only when the column exists. Safe via
-- sql-batch-dbs.ts against every region (regional DBs hold item_l/item_no;
-- item master lives in imartap + regional replica). Re-running is a no-op.
