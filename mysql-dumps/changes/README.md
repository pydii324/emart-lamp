...
- **`ALL` (албански лек) не е в `02`** — той е част от baseline-а (старият ENUM е
  `('BGN','EUR','ALL','RON')`, редът идва от `deploy/07`), тъй че `01` го ре-базира
  заедно с всичко останало: `0.01960000 BGN ÷ 1.95583 = 0.01002132 EUR`. В `02` има
  закоментиран единичен `INSERT` за инстанция, която наистина няма реда — вкарването
  му в общия `INSERT` би дало `ERROR 1062` и би отменило и другите 14 реда.
- **BNB не публикува лек** — проверено срещу живия feed (29 валути, `ALL` го няма), тъй
  че `scripts/update-currency-rates.php` никога не пипа този ред. Реалният курс се слага
  ръчно от БНБ на Албания.
- **`RUB` и `UAH` са `source = 'bnb'`, но BNB вече не ги листва.** Cron-ът ще логва
  `WARN: provider had no rate for: RUB, UAH` на всеки run и ще ги оставя на последната
  стойност — това е проектираното поведение за изпаднала валута, не грешка. Ако шумът
  пречи, сложи ги на `manual` (в `02`, `fresh/09` и `deploy/10` едновременно, иначе
  инстанциите се разминават).
