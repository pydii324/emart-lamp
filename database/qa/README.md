...
- **`VOUCHER500ALL`** — само старият ред със `site='bg'` и `currency='ALL'`. Остатък
  от конверсионния слой, махнат на 10.09; нарушава инварианта „един регион = една
  валута". От 21.09.2026 `fresh/08` го seed-ва като `site='al'` и в този вид е
  коректен — затова се трие по `site`, не по код.
