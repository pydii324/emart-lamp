...
--
-- Всички кодове са `site='bg'`, `currency='EUR'` (PromoRegion::CURRENCY['bg']),
...
-- Два реда от `fresh-seed`, които пречат на тестването:
--   SHIPPCT50     — type 'shipping_percent'. Типът пада изцяло (решение от
--                   20.09.2026); махнат е от DB ENUM-а на 21.08 и предстои да
--                   отпадне и от PHP. Код от този тип не бива да се тества.
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QAINACTIVE', 'percent', NULL, 10.00, 'EUR',  0.00, NULL, 0, 0, 0, NULL, '20240101000000', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: active=0 И изтекъл -> "Кодът не е активен" (active бие expired)', 'bg', 'qa-seed'),
  ('QAEXPIRED',  'percent', NULL, 10.00, 'EUR',  0.00, NULL, 0, 0, 1, NULL, '20240101000000', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: изтекъл -> "Кодът е изтекъл"', 'bg', 'qa-seed'),
  ('QAMIN50',    'percent', NULL, 10.00, 'EUR', 50.00, NULL, 0, 0, 1, NULL, NULL,             DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: min_subtotal -> "Минимална сума за код: 50.00 <валута>"', 'bg', 'qa-seed'),
  ('QAUSEDUP',   'percent', NULL, 10.00, 'EUR',  0.00, NULL, 1, 1, 1, NULL, NULL,             DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: вече изчерпан -> "Кодът е изчерпан"', 'bg', 'qa-seed'),
  ('QARO',       'percent', NULL, 10.00, 'RON',  0.00, NULL, 0, 0, 1, NULL, NULL,             DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: чужд регион -> "Невалиден код" (умишлено неразличимо от несъществуващ)', 'ro', 'qa-seed');
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QASTACKA', 'percent', NULL, 10.00, 'EUR', 0.00, NULL, 0, 0, 1, 1, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: stack_group 1, първи', 'bg', 'qa-seed'),
  ('QASTACKB', 'percent', NULL, 15.00, 'EUR', 0.00, NULL, 0, 0, 1, 1, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: stack_group 1, втори -> конфликт', 'bg', 'qa-seed');
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QAPCT10', 'percent', NULL, 10.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: 10% — по-малкият от двойката', 'bg', 'qa-seed'),
  ('QAPCT20', 'percent', NULL, 20.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: 20% — печели при двойка percent кодове', 'bg', 'qa-seed');
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QAVOUCHER5',   'fixed', 'voucher',   5.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: ваучер 5.00 -> отрицателен ред SKU 5555555', 'bg', 'qa-seed'),
  ('QACOUPON5',    'fixed', 'coupon',    5.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: купон 5.00 -> отрицателен ред SKU 7777777', 'bg', 'qa-seed'),
  ('QAVOUCHERBIG', 'fixed', 'voucher', 500.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: ваучер 500.00 -> капва се на количката, изгаря цял, прелива върху доставката', 'bg', 'qa-seed');
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QAFREESHIP',  'shipping', NULL, 0.00, 'EUR', 0.00, NULL, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: без таван -> цялата доставка пада', 'bg', 'qa-seed'),
  ('QASHIPCAP3',  'shipping', NULL, 0.00, 'EUR', 0.00, 3.00, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: таван 3.00 -> сваля най-много 3.00', 'bg', 'qa-seed'),
  ('QASHIPCAP0',  'shipping', NULL, 0.00, 'EUR', 0.00, 0.00, 0, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: таван 0.00 -> не сваля НИЩО (различно от NULL)', 'bg', 'qa-seed');
...
INSERT INTO `promo_codes`
  (`code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`, `shipping_cap`, `max_uses`, `times_used`, `active`, `stack_group`, `expiration_date`, `created_at`, `source`, `note`, `site`, `created_by`) VALUES
  ('QAONESHOT', 'fixed', 'voucher', 5.00, 'EUR', 0.00, NULL, 1, 0, 1, NULL, NULL, DATE_FORMAT(NOW(), '%Y%m%d%H%i%s'), 'manual', 'QA: max_uses=1 -> за финализиране и race теста', 'bg', 'qa-seed');

-- --- Проверка ----------------------------------------------------------------
SELECT `id`, `code`, `type`, `subtype`, `discount_value`, `currency`, `min_subtotal`,
