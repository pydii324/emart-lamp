...
- **09 няма подредбена зависимост.** `currency_rates` вече не се чете от PHP-то —
  промо кодовете не се конвертират (виж „Една валута на регион" по-долу). Таблицата
  се създава само защото пази курсовете, при които са писани историческите редове.
...
- Нов регион = **три** промени, в този ред: ENUM-ът тук → `PromoCode::SITES` →
  ...
  трите таблици. Ред в `currency_rates` вече НЕ е нужен — нищо не конвертира.
- **Една валута на регион — и нула конверсия.** Количката на региона (`item_l`/`item_no`)
  е в неговата валута: каталогът е в EUR, но цените се конвертират на влизане през
  `kurs` (`txt_kurs_valuta` / per-region kurs файла), което е и това, което
  `porachki.valuta` + `porachki.kurs` записват. Промо кодовете се пишат в същата валута,
  тоест **валута на кода == валута на количката == валута на региона**. Затова всяка
  парична колона в `cart_promo_codes` / `order_promo_codes` — включително
  `discount_applied` — е във валутата от `currency` на реда, записана както е в
  `imartap.promo_codes`. **BGN не изчезва** — остава в ENUM-а, защото старите BGN кодове
  и минали поръчки още го сочат, но нито един регион вече не пише в него.
